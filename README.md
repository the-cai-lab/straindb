...
The Perl script `validate.pl` is used to validate data that has been uploaded by a user using the "Batch Create" feature.  This is available for all data tables.

Besides performing rudimentary checks such as whether the date is properly formatted, it also consults the database if a data table depends on another data table.

For example, when uploading to the "yeasts" table the "mating type" has to be validated against the "mating_types" table.  And the string describing the mating type is converted to the primary key of the "mating_types" table.

Checks like this are performed by `validate.pl`, with a clean data table returned to the system for upload.  A report is also generated by the system as a text file; the Strain Database application shows this report to the user, to help indicate the row and value that is incorrectly formatted.
